...
<div class="container-fluid">
  <div class="row">
    ...
    <main class="col-md-9 col-lg-10">
      <div class="col-lg-6 col-md-8 mx-auto mt-5"> <!-- เพิ่ม mt-4 เพื่อเพิ่มระยะห่างด้านบน -->
        <form class="d-flex align-items-center col-12 col-lg-auto mb-3 mb-lg-0 me-lg-3" 
              role="search" action="indexproductadmin.php" method="GET">
 ...
 <div class="album py-5 bg-body-tertiary">
  <div class="container">
  <h1 class="fw-light" style="font-weight: bold !important;">Products</h1>

   <div class="d-flex justify-content-end align-items-center">
  <a href="insertadmin.php" class="btn btn-success" style="padding: 6px 15px; font-size: 0.9rem; margin-right: 15px;">
    <i class="bi bi-upload"></i> เพิ่มสินค้า 
  </a>
	  </div>
	</div>
  </div>
              ...
              <div class="row row-cols-1 row-cols-sm-2 row-cols-md-4 g-4">
                  ...
                  <?php
                      ...
                      // ใช้ $_GET แทน $_POST สำหรับการค้นหา
                      $kw = isset($_GET['search']) ? trim($_GET['search']) : '';

                      // ตรวจสอบว่ามีการค้นหาหรือไม่
                      if ($kw !== '') {
                          // ใช้ prepared statements เพื่อป้องกัน SQL Injection
                          $stmt = $conn->prepare("SELECT * FROM product WHERE p_name LIKE CONCAT('%', ?, '%') OR p_detail LIKE CONCAT('%', ?, '%')");
                          if ($stmt) {
                              ...
                              if ($result->num_rows > 0) {
                                  while ($data = $result->fetch_assoc()) {
                                      ...
                                      $imagePath = "images/" . htmlspecialchars($data['p_id']) . "." . htmlspecialchars($data['p_picture']);
                  ...
                  <div class="col d-flex align-items-stretch">
                      <div class="card shadow-sm w-100">
                         <img src="<?= $imagePath; ?>" class="product-image card-img-top img-fluid" 
                               alt="<?= htmlspecialchars($data['p_name'], ENT_QUOTES, 'UTF-8'); ?>">
                          <div class="card-body d-flex flex-column">
                              <p class="card-text mb-4">
                                  <?= htmlspecialchars($data['p_name'], ENT_QUOTES, 'UTF-8'); ?>
                              </p>
                              <div class="d-flex justify-content-between align-items-center mt-auto">
                                  <span>
                                      <?= number_format($data['p_price'], 2); ?> บาท
                                  </span>
                                  <a href="updateadmin.php?pid=<?= urlencode($data['p_id']); ?>" class="btn btn-warning">
                            <i class="bi bi-pencil-square"></i> แก้ไข
                        </a>
                        <a href="deleteadmin.php?pid=<?=$data['p_id'];?>&picture=<?=$data['p_picture'];?>" 
                           onClick="return confirm('ยืนยันการลบ?');" class="btn btn-danger">
                            <i class="bi bi-backspace-fill"></i> ลบ
                        </a>
                              </div>
                          </div>
                      </div>
                  ...
                  <?php 
                          ...
                          // กรณีไม่มีการค้นหา แสดงสินค้าทั้งหมด
