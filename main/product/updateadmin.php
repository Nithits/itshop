...
<style>
		<style>
		.navbar-brand {
      font-size: 10000rem !important; /* ปรับขนาดฟอนต์ตามที่ต้องการ */
  }
        /* สไตล์ที่คุณมีอยู่ */
	.btn {
  ...
  background-color: #999999;
	...
	.form-label {
		font-size: 0.9rem; /* ปรับขนาดฟอนต์ของ label */
	}
	
	.form-control {
		font-size: 0.9rem; /* ปรับขนาดฟอนต์ของ input และ textarea */
	}

  .navbar .text-white {
      color: FFF !important; /* เปลี่ยนสีฟอนต์ใน navbar เป็นสีดำ */
  }
  ...
  .sidebar .nav-link:hover {
      color: #555; /* สีฟอนต์เมื่อเอาเมาส์ไปวางที่ลิงค์ */
  }
 ...
 .form-container {
		...
		.form-group {
            margin-bottom: 15px; /* เพิ่มระยะห่างระหว่างกลุ่มฟอร์ม */
        }
		...
		textarea {
            resize: none; /* ไม่ให้สามารถเปลี่ยนขนาด textarea */
        }

        button {
            width: 100%; /* ทำให้ปุ่มเต็มความกว้าง */
            padding: 10px; /* เพิ่ม padding ปุ่ม */
        }

        button[type="submit"] {
            background-color: #28a745; /* สีพื้นหลังของปุ่มบันทึก */
            color: white; /* สีข้อความของปุ่มบันทึก */
        ...
        .text-center h1 {
            margin-bottom: 20px; /* ระยะห่างระหว่างหัวข้อและฟอร์ม */
        }
		.form-label {
            font-weight: bold; /* ทำให้ฟอนต์เข้ม */
			text-align: left; /* จัดตำแหน่งข้อความให้ชิดซ้าย */
            display: block; 
        }
    </style>
...
<main class="col-md-9 col-lg-10">
        ...
        <div class="form-container">
            <form method="post" action="" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" id="pname" name="pname" class="form-control" required autofocus value="<?=$data1['p_name'];?>">
                </div>

                <div class="form-group">
                    <label for="pdetail" class="form-label">รายละเอียดสินค้า</label>
                    <textarea id="pdetail" name="pdetail" class="form-control" rows="5"><?=$data1['p_detail'];?></textarea>
                </div>

                <div class="form-group">
                    <label for="pprice" class="form-label">ราคา</label>
                    <input type="text" id="pprice" name="pprice" class="form-control" required value="<?=$data1['p_price'];?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="pcat" name="pcat" class="form-select">
                        <?php
    ...
    <button type="submit" name="Submit" class="btn btn-success" style="padding: 6px 15px; width: 150px;">
        <i class="bi bi-save"></i> บันทึก
    </button>
    <a class="btn btn-secondary" href="indexproductadmin.php" style="padding: 6px 15px; font-size: 0.9rem; width: 150px;">
...
if(isset($_POST['Submit'])) {
    if(empty($_FILES['pimg']['name'])) {
        $sql = "UPDATE `product` SET `p_name` = '{$_POST['pname']}', `p_detail` = '{$_POST['pdetail']}', `p_price` = '{$_POST['pprice']}', `pt_id` = '{$_POST['pcat']}' WHERE `product`.`p_id` = '{$_GET['pid']}';";
    } else {
        $file_name = $_FILES['pimg']['name'];
        $picture = substr($file_name, strpos($file_name, '.') + 1);
        $sql = "UPDATE `product` SET `p_name` = '{$_POST['pname']}', `p_detail` = '{$_POST['pdetail']}', `p_price` = '{$_POST['pprice']}', `pt_id` = '{$_POST['pcat']}', p_picture='{$picture}' WHERE `product`.`p_id` = '{$_GET['pid']}';";
        copy($_FILES['pimg']['tmp_name'], "images/".$_GET['pid'].".".$picture);
    }

    mysqli_query($conn, $sql) or die ("แก้ไขข้อมูลสินค้าไม่ได้");
    echo "<script>alert('แก้ไขข้อมูลสินค้าสำเร็จ'); window.location='indexproductadmin.php';</script>";
}
?>
